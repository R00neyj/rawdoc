...
// 표 원문 범위 안에 있는 빈 커서인가 (F-139 3.1). 표를 치는 도중(구분 행이
...
// 유지) — main.empty 로 가른다.
// 칸 편집 중에는(tableWidget.ts startEdit·enterTableFromKeyboard) 주 에디터
...
// 차원에서 멈춰 있었던 것이었다(탭을 화면에 그려지게(스크린샷 등으로 페인트를
...
class CodeWidget extends WidgetType {
  ...
  // info: CodeInfo(언어). lines: 각 줄의 원문(들여쓰기·"> " 접두 제외)과 블록 시작
  // 기준 상대 offset(그 줄 첫 글자 위치). 목록·인용 안 코드블록은 줄마다 CodeText 가
  // 나뉘어(F-134 3.4) offset 이 등차수열이 아닐 수 있어 줄별로 든다
...
// hasFocus: 편집기 포커스 (F-146 3.2). 포커스가 없으면 겹침(커서가 원문 안)을
// 무시하고 항상 위젯(접힌 프리뷰)으로 본다 — "코드블록·표 원문 펼침"도 비포커스
