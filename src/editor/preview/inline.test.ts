...
import { buildInline, mapDecorationsOnHold } from './inline'

function makeState(doc: string, anchor = 0, head = anchor) {
...
function hiddenLength(state: CMState): number {
  return hideRanges(state).reduce((sum, r) => sum + (r.to - r.from), 0)
}

function hiddenCount(state: CMState): number {
...
function linkMarksOf(state: CMState) {
