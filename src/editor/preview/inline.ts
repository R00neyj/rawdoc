...
// 인라인 순회가 들어가지 않는 블록 — 표·펜스 코드블록은 통째로 원문으로 둔다. 이것이 없으면
// 펜스 코드블록의 울타리(CodeMark)가 인라인 코드의 백틱과 같은 이름을 써서 오탐이 난다
...
// 커서가 없는 줄에서 숨기되, 부모가 특정 노드일 때만 숨기는 마커 노드 — 부모 확인 없이
// 숨기면 `<https://…>`(Autolink)·Image 안의 URL 도 함께 사라진다. 판정 단위는 줄이다
const LINE_GATED_MARK: Record<string, string> = {
  CodeMark: 'InlineCode',
}
...
function linkMarks(linkNode: SyntaxNode): SyntaxNode[] {
...
// 읽는다 — 그대로 두면 이 파일의 일반 규칙이 [ ] 를 숨긴다. lines.ts 의 콜아웃
// 판정(부모가 Blockquote 가 아닌 인용만 후보 — F-128 2장 "중첩은 바깥만")과 같은
// 규칙을 여기서도 적용한다
function isCalloutHeaderLink(state: EditorState, linkNode: SyntaxNode): boolean {
...
// 이 파일의 일반 규칙대로 그 Link 의 LinkMark([ ])를 숨기면 wikiLinks.ts(F-131)가
// 그리는 바깥 [[ ]] 숨김·표시와 겹친다 — 이 Link 노드가 실제 위키링크 범위 안이면
// (findWikiLinks 로 판정) 이 파일은 손대지 않는다(F-131 1장 "필요할 때만")
function isInsideWikiLink(state: EditorState, linkNode: SyntaxNode): boolean {
...
// ranges 는 보통 view.visibleRanges. hasFocus 는 편집기 포커스 (F-146 3.2).
// 기본값 true 는 포커스를 다루지 않는 기존 호출부(테스트 등)의 동작을 그대로 유지한다
...
// 조합 중 보류할 때 문서가 바뀌었으면 decoration 위치를 따라간다 (F-134 3.1). 옛 decoration 을
// 옛 위치 그대로 두면 새 문서에서 다른 글자(줄바꿈 포함)를 가리킬 수 있어 CM 이 RangeError 를
// 던진다(blocks.ts 는 이미 이렇게 한다) — 그래서 이 map 분기만 따로 뗀 함수다
...
export function inlinePreview(): Extension {
