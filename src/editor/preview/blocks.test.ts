...
type TestCodeLine = { text: string; offset: number }
type TestCodeWidget = { info: string; lines: TestCodeLine[]; eq(other: TestCodeWidget): boolean }
type TestImageWidget = {
  id: string
  ext: string
  alt: string
  align: string
...
describe('observeHeight/stopObservingHeight — DOM 요소 기준 추적 (F-134 3.6)', () => {
  it('stopObservingHeight(el) 은 그 el 에 연결된 ResizeObserver 를 해제한다', () => {
    ...
    try {
      ...
      observeHeight(el, { requestMeasure: vi.fn() } as unknown as EditorView)
...
describe('blockPreview — 구문 트리만 바뀐 갱신 (F-134 3.8)', () => {
  function widgetCount(state: CMState): number {
    const decos = state.facet(EditorView.decorations).find((e) => typeof e !== 'function') as
