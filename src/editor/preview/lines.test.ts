...
function makeStateWithFrontmatter(doc: string, anchor = 0, head = anchor): EditorState {
  const state = EditorState.create({
    doc,
    selection: { anchor, head },
...
function build(state: EditorState): CMRange<Decoration>[] {
  return buildLines(state, [{ from: 0, to: state.doc.length }])
}

function lineClasses(ranges: CMRange<Decoration>[]): string[] {
  return ranges.filter((r) => r.value.spec.class).map((r) => r.value.spec.class)
}

function replaced(ranges: CMRange<Decoration>[]): CMRange<Decoration>[] {
...
describe('fenceLineRanges — 펼친 코드블록 (F-124 3.4 11번)', () => {
  ...
  function fenceRanges(state: EditorState) {
...
describe('buildLines — 콜아웃 (F-128 5장 A2)', () => {
  function classesOf(decos: CMRange<Decoration>[], lineNumber: number, doc: EditorState['doc']): string {
  ...
  function typeMarks(decos: CMRange<Decoration>[]) {
...
describe('buildLines — 콜아웃 아이콘 (F-148 4장 A1)', () => {
  function iconWidgets(decos: CMRange<Decoration>[]) {
