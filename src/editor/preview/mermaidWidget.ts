// 편집 모드 Mermaid 다이어그램 블록 위젯 (specs/features/F-258.md 2.3). F-106 코드블록 패턴을 따르고, 클릭 진입은 blocks.ts 의 enterOnClick 을 그대로 쓴다
...
// 주 EditorView → Map<소스, Promise<결과>> (F-258 2.3) — 같은 소스를 여러 번 렌더링하지 않는다. blocks.ts 의 tracker 가 destroy() 에서 destroyMermaidCache(view) 를 부른다
...
function loadMermaid(view: EditorView, source: string): Promise<MermaidResult> {
  const cache = getCache(view)
  const cached = cache.get(source)
  if (cached) return cached
  const promise = renderMermaid(source)
  cache.set(source, promise)
  return promise
}

// 펜스 코드블록 위젯(mermaid 정보 문자열, F-258 2.1). eq() 비교 키는 원문 소스 문자열(코드 본문)만이다 — 위치·정보문자열은 넣지 않는다(F-258 2.3)
export class MermaidWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    loadMermaid(view, this.source).then((result) => {
