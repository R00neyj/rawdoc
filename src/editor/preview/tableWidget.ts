...
type PendingFocus = { wrap: HTMLElement; row: number; col: number; restore?: ScrollRestore }
type ScrollRestore = { scrollTop: number; scrollLeft: number }
type ActiveRangeEntry = { wrap: HTMLElement; r1: number; c1: number; r2: number; c2: number }
type DeferredWrite = { range: CellRange; wasNonEmpty: boolean; value: string }
...
const activeRange = new WeakMap<EditorView, ActiveRangeEntry>()
...
function clampIntoView(container: HTMLElement, targetRect: DOMRect, axis: 'x' | 'y'): void {
...
function focusCellClamped(
  ...
  const hScroll = wrap.querySelector<HTMLElement>('.md-table-scroll')
  const run = () => {
    if (restore) {
...
function cellKeydown(mainView: EditorView, wrap: HTMLElement, cellView: EditorView, event: KeyboardEvent): boolean {
  if (event.isComposing || event.keyCode === 229 || isComposing(cellView)) return false

  const mod = event.ctrlKey || event.metaKey
  const entry = () => activeEdit.get(mainView)

  function moveTo(row: number, col: number): boolean {
    const e = entry()
    if (!e) return false
  ...
  function consume(handled: boolean): boolean {
    if (handled) {
  ...
  if (event.key === 'Enter') {
    ...
    if (e.row + 1 >= e.widget.table.rows.length) {
      ...
      const blockFrom = mainView.posAtDOM(wrap)
      const { changes } = addRow(latest.table) as { changes: TableChange[] }
...
const CELL_MARK_CLASS: Record<string, string> = {
  strong: 'md-strong',
  em: 'md-em',
  strike: 'md-strike',
  code: 'md-code',
...
function beginPointerSelection(mainView: EditorView, wrap: HTMLElement, startRow: number, startCol: number): void {
  ...
  let moved = false
  let current: CellHit = { row: startRow, col: startCol }
  ...
  function onUp(): void {
    ...
    if (moved) {
      ...
      if (widget) startEdit(mainView, wrap, widget, startRow, startCol)
    }
  }
...
function buildCell(
  ...
  el.addEventListener('keydown', (event) => {
    ...
    event.preventDefault()
    const widget = currentWidgetFor(wrap)
    if (widget) startEdit(mainView, wrap, widget, row, col)
  })
...
const renderedWidget = new WeakMap<HTMLElement, TableWidget>()

function currentWidgetFor(wrap: HTMLElement): TableWidget | undefined {
...
function positionAddButtons(wrap: HTMLElement): void {
  ...
  const table = wrap.querySelector('table')
  const rowBtn = wrap.querySelector<HTMLElement>('.md-table-add-row')
  const colBtn = wrap.querySelector<HTMLElement>('.md-table-add-col')
...
function getCellHighlight(wrap: HTMLElement): HTMLElement | null {
  ...
  let el = scroll.querySelector<HTMLElement>('.md-table-cell-highlight')
  if (!el) {
...
function positionCellHighlight(wrap: HTMLElement, td: HTMLTableCellElement, { visible = true }: { visible?: boolean } = {}): void {
...
function hideCellHighlight(wrap: HTMLElement): void {
  const el = wrap.querySelector<HTMLElement>('.md-table-cell-highlight')
  if (el) el.hidden = true
}

// 강조를 (r1,c1)-(r2,c2) 두 칸을 감싼 사각형(바깥 칸 테두리 안쪽)으로 그린다(F-165 2.1). 좌표 순서는 무관 — 여기서 정규화한다
function renderRangeHighlight(wrap: HTMLElement, r1: number, c1: number, r2: number, c2: number): boolean {
  const el = getCellHighlight(wrap)
  const table = wrap.querySelector('table')
...
function clampCellFromPoint(wrap: HTMLElement, clientX: number, clientY: number): CellHit | null {
  const table = wrap.querySelector('table')
...
function detachOutsideClickListener(mainView: EditorView): void {
...
function attachOutsideClickListener(mainView: EditorView, wrap: HTMLElement): void {
  detachOutsideClickListener(mainView)
  const handler = (event: MouseEvent) => {
    if (wrap.contains(event.target as Node | null)) return
    clearRangeSelection(mainView)
  }
...
function clearRangeSelection(mainView: EditorView | undefined | null): void {
  if (!mainView) return
...
function rangeHighlightKeydown(mainView: EditorView | undefined, wrap: HTMLElement, event: KeyboardEvent): void {
  if (!mainView) return
  const range = activeRange.get(mainView)
...
function finalizeRangeSelection(mainView: EditorView, wrap: HTMLElement, r1: number, c1: number, r2: number, c2: number): void {
...
function performRangeDelete(mainView: EditorView, wrap: HTMLElement, range: ActiveRangeEntry): void {
  ...
  if (kind === 'table') {
    changes = deleteTable(table) as TableChange[]
  } else if (kind === 'columns') {
    changes = deleteColumns(table, indicesBetween(c1, c2)) as TableChange[]
  } else if (kind === 'rows') {
    changes = deleteRows(table, indicesBetween(r1, r2)) as TableChange[]
    ...
    changes = [...(clearCells(table, headerCells) as TableChange[]), ...(deleteRows(table, bodyRows) as TableChange[])]
  } else {
    const cells: CellHit[] = []
    for (let r = r1; r <= r2; r++) {
      for (let c = c1; c <= c2; c++) cells.push({ row: r, col: c })
    }
    changes = clearCells(table, cells) as TableChange[]
  }
...
function observeButtonPosition(wrap: HTMLElement): void {
  const table = wrap.querySelector('table')
  const scroll = wrap.querySelector('.md-table-scroll')
...
function stopObservingButtonPosition(wrap: HTMLElement): void {
  positionObservers.get(wrap)?.disconnect()
  positionObservers.delete(wrap)
}

// "뒤에 행/열 추가하기" 버튼 1개
function buildAddButton(className: string, label: string, onClick: (event: MouseEvent) => void): HTMLButtonElement {
...
function renderTable(wrap: HTMLElement, widget: TableWidget, mainView: EditorView): void {
  wrap.innerHTML = ''
  ...
  wrap.appendChild(
    buildAddButton('md-table-add-col', '뒤에 열 추가하기', () => {
      ...
      if (!latest) return
      const blockFrom = mainView.posAtDOM(wrap)
      const { changes } = addColumn(latest.table) as { changes: TableChange[] }
      if (changes.length === 0) return
...
function sameShape(dom: HTMLElement, table: TableModel): boolean {
...
// GFM 표 위젯. 칸 편집·행/열 추가는 tableModel.ts 계산 + 하위 EditorView 로 한다
export class TableWidget extends WidgetType {
  text: string
  table: TableModel

  // text: 표 블록 전체 원문(문서 절대 위치 아님 — 블록 시작 기준)
