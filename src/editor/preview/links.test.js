...
import { clickTargetIsLinkText, findLinkAt, isOpenableUrl } from './links.js'
...
describe('clickTargetIsLinkText (F-134 3.2)', () => {
  it('target 이 .md-link 표시 요소 안이면 true', () => {
    const target = { closest: (sel) => (sel === '.md-link' ? {} : null) }
  ...
  it('target 이 없거나 closest 가 없으면 false', () => {
    ...
    expect(clickTargetIsLinkText(undefined)).toBe(false)
    expect(clickTargetIsLinkText({})).toBe(false)
  })
})
