...
import { buildImageBlock } from '../lib/imageBlock.js'
...
export function isBlockedPosition(state, pos) {
...
export function computeImagePlacement(docText, basePos, blockTexts) {
...
function isViewOnly(view) {
  return view.dom.dataset.view === 'view'
}

function measureContentWidth(view) {
...
function waitForCompositionEnd(view) {
...
const setTrackedPos = StateEffect.define()
const trackedPosField = StateField.define({
  create: () => null,
  update(value, tr) {
...
async function handleIncomingImages(view, files, { source, pos, onImageFiles }) {
...
// 붙여넣기·끌어놓기 이미지 받기(2.4·2.5). createEditor.js 가 onImageFiles(files, {source, blocked?}) => Promise<attached[]> 옵션과 함께 조립한다
export function imageInsert({ onImageFiles } = {}) {
  return [
    trackedPosField,
