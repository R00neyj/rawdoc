...
function countRun(state, pos, ch, dir) {
  const line = state.doc.lineAt(pos)
  let count = 0
  let i = pos
  if (dir < 0) {
...
function insideOpaqueNode(state, pos) {
 ...
 * 따로 확인하지 않는다.
 */
function isEmptyPair(state, pos, sym) {
 ...
 * (F-127 3.3 백틱 추가 규칙)
 */
function isEmptyDoubleBacktickLine(state, pos) {
  const line = state.doc.lineAt(pos)
...
function bracketOpenRule(state, pos, open) {
...
function bracketCloseRule(state, pos, closeChar) {
  const next = charAfter(state, pos)
...
function spaceInEmptyStarPair(state, pos) {
...
function emphasisRule(state, pos, sym) {
 ...
 */
export function autoPairInput(state, from, to, text) {
 ...
 * @returns {import('@codemirror/state').TransactionSpec | null}
 */
export function autoPairBackspace(state) {
  if (state.readOnly) return null
 ...
 * createEditor.js 에서 markdown() 보다 앞서 조립해 같은 Prec.high 안에서 순서를 이긴다 */
function backspaceCommand(view) {
