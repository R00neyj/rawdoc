...
import { frontmatterExtension, frontmatterWidgetExtension, frontmatterWidgetInfo } from './frontmatter'
...
describe('frontmatterExtension — 블록 파서 (A2)', () => {
  ...
  it('Frontmatter 는 여는·닫는 FrontmatterMark 를 자식으로 가진다', () => {
    ...
    let frontmatter: SyntaxNode | null = null
    tree.iterate({
      enter: (n) => {
    ...
    const found = frontmatter as SyntaxNode | null
    const marks: string[] = []
    for (let c = found?.firstChild; c; c = c.nextSibling) marks.push(c.name)
...
describe('frontmatterWidgetExtension — 커서 보정 (F-155 2.2, A3)', () => {
  function makeCorrectionState(doc: string) {
