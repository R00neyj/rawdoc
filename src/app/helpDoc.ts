...
왼쪽 사이드바에서 문서와 폴더를 관리합니다. 자주 쓰는 문서는 위쪽 \`고정됨\` 묶음에 모아 둘 수 있고, \`검색\`은 아직 준비 중입니다. 위쪽 상단바에서 보기 모드를 바꾸고 \`.md 내보내기\`·공유·계정 메뉴를 엽니다. 아래쪽 상태바에는 글자 수·단어 수와 저장 상태가 보입니다. 넓은 창에서는 오른쪽에 지금 문서의 목차가 뜨고, 좁은 창에서는 버튼을 눌러 카드로 봅니다.
