...
> **이 파일은 2026-09-21 시점 스냅샷이다. 원본은 `.claude/skills/ship-feature/SKILL.md`(영어).**
...
**3D 지도 재작업은 소명세마다 갈리고, `specs/features/F-292.md` 9장의 `누가` 열이 그 결정이다** (사용자, 2026-09-21). **F-2002~F-2004** 는 메인이 직접 구현한다 — 렌더러 교체·카메라 조작·노드 표현은 화면을 보면서 고쳐야 하고 F-2002 는 SVG 삭제까지 안고 있어서, 에이전트를 거치는 왕복이 아끼는 것보다 비싸다. **F-2006**(장력 슬라이더)은 `complex-implementer` 로 간다 — 범위를 손맛으로 정해야 하고 G7 게이트를 안고 있다. 나머지는 평범하게 맡긴다. 이 문서의 나머지는 메인이 직접 하는 커밋에도 그대로 적용된다 — 테스트 먼저, `npm run review -- F-xxx`, 한 명세 한 커밋.
