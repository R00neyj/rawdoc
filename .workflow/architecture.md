...
- [ ] **IME 조합 중 decoration 재계산 보류는 "데스크톱 Chrome 에서는" 불변조건이 아니다.**
      T-003(2026-09-08, B-002)에서 실측했다. 보류 ON 과 OFF 어느 쪽에서도
      커서 튐·글자 중복이 없었고 문서 원문도 정확했다. 보류가 없어도 결과가 같았다.
      *(원래 서술: "`compositionstart` ~ `compositionend` 사이에는 뷰 갱신을 보류한다" — 검증 대상 가설이었다)*

      **다만 삭제하지 않고 범위를 좁혀 남긴다.** 이유 셋:
      1. 안드로이드 Chrome 과 Capacitor WebView 는 아직 검증되지 않았다. 아래 마지막
         불변조건("한 환경 통과를 전체 통과로 적지 않는다")이 그대로 적용된다. **B-005·B-006 에서 다시 판정한다**
      2. 보류는 실제로 일하고 있었다 — 조합 1회당 재계산 2~5건, 총 17건이 걸러졌다.
         데스크톱에서 그것이 없어도 멀쩡했다는 것이지, 아무 일도 안 했다는 뜻이 아니다
      3. 지금 제거하면 안드로이드에서 재검증할 수단이 사라진다. spike 는 보류를 기본 ON 으로 유지한다

      **함께 확인된 사실 — `compositionstart` 시점의 `view.composing` 은 `false` 다.**
      그 시점에 `true` 인 것은 `compositionStarted` 뿐이다. 즉 `composing` 을 기준으로
      보류를 걸면 **조합 진입 직후 첫 update 1건은 보류를 타지 않고 통과한다.**
      데스크톱에서는 이 1건이 아무 문제를 일으키지 않았다.
      안드로이드에서도 그런지가 B-005 의 관찰 대상이다 — `compositionStarted` 로 바꾸면
      이 1건은 막히지만, 안드로이드에서 상시 true 에 가까워져 프리뷰가 멈출 위험이 있다(4장 참조)
