...
- **These were broken, not flaky. Found during F-290 on 2026-09-21, traced and fixed on the same day (`5dc3ac4`)**
  ...
  - **Still failing**: `F-281 A14` — the `전체 내보내기` button does not become disabled when offline (`exportOffline = store.kind === 'server' && syncState?.online === false`; the test's `server.setOffline(true)` does not reach it). Product behavior is suspect, not the selector
  - **Still failing**: `F-146 A8` (clicking a link with marks hidden — the `popup` event never arrives). Already failing at `036e093`; anything earlier cannot be checked with the current `node_modules` because JetBrains Mono was still a dependency then
