...
**지금은 화면에서 계정을 지우는 기능이 없습니다.** 계정과 그 계정의 문서를 모두 지우고 싶으면 13절 연락처로 알려 주세요. 본인 확인 뒤 지우고 처리 결과를 알려 드립니다.
...
- 그 밖의 요청과 계정 삭제는 13절 연락처로 받습니다. 법정대리인도 같은 방법으로 요청할 수 있습니다.
...
| `md-docs`, `md-remote` | 인덱스드DB | 로그인하지 않았을 때의 문서와, 로그인했을 때 서버 문서의 사본. 인터넷이 끊겨도 쓸 수 있게 둡니다. 금고 문서와 금고 키 묶음은 암호화한 값으로 둡니다 | 지울 때까지 |
| `md-yjs` | 인덱스드DB | 로그인했을 때 서버 문서의 편집 기록. 인터넷이 끊긴 동안 고친 내용을 다시 연결될 때 올리기 위해 둡니다 | 서버에 다 올라간 문서는 마지막으로 연 날부터 30일 뒤 지웁니다. 서버에서 지워진 문서는 곧바로 지웁니다. 로그아웃해도 남습니다 |
